    ...
    .then((data) => {
      ...
      for (var i = 0; i < 25; i++) {
        document.getElementById("data").innerHTML =
          ...
          <td class="px-6 py-4 MSRRGN_NM">${data.RealtimeCityAir.row[i].MSRRGN_NM}</td>
          <td class="px-6 py-4 MSRSTE_NM">${data.RealtimeCityAir.row[i].MSRSTE_NM}</td>
          <td class="px-6 py-4 IDEX_NM">${data.RealtimeCityAir.row[i].IDEX_NM}</td>
          <td class="px-6 py-4 PM10">${data.RealtimeCityAir.row[i].PM10}</td>
          <td class="px-6 py-4 PM25">${data.RealtimeCityAir.row[i].PM25}</td>
          <td class="px-6 py-4 O3">${data.RealtimeCityAir.row[i].O3}</td>
          <td class="px-6 py-4 SO2">${data.RealtimeCityAir.row[i].SO2}</td>
          <td class="px-6 py-4 IDEX_MVL">${data.RealtimeCityAir.row[i].IDEX_MVL}</td>
        </tr>`;

        IDEX_MVL_Arr.push({
          [`row_${i}`]: data.RealtimeCityAir.row[i].IDEX_MVL,
        });
      }
